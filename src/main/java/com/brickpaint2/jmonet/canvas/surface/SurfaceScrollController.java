package com.brickpaint2.jmonet.canvas.surface;

import javax.swing.*;
import java.awt.*;

/**
 * A mechanism enabling a surface to request that its scrolled position (within a {@link JScrollPane}) be
 * changed.
 *
 * This interface allows tools to advise scroll position changes to the {@link JScrollPane} (or other
 * container) holding the surface. This enables changes to scale to intelligently "zoom in" on the area being scaled and
 * for tools like the {@link com.brickpaint2.jmonet.tools.MagnifierTool} to recenter the scroll position.
 */
